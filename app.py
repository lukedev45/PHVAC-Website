...
How to run:
1) Install deps:  
   pip install fastapi uvicorn sqlmodel jinja2 passlib[bcrypt] python-multipart
      ...
      <nav class="flex items-center gap-4">
        {% if current_user %}
          ...
          <form method="post" action="/logout">
            <button class="text-sm px-3 py-1 rounded bg-gray-100 hover:bg-gray-200">Logout {{ current_user.full_name }}</button>
          </form>
        {% else %}
          <a class="text-sm px-3 py-1 rounded bg-gray-100" href="/login">Login</a>
        {% endif %}
      </nav>
    </div>
  </header>
...
<div class="max-w-md mx-auto bg-white p-6 rounded-xl shadow">
  ...
  <p class="text-xs text-gray-500 mt-4">No users yet? Go to <a class="underline" href="/bootstrap">/bootstrap</a> to create the first manager.</p>
</div>
{% endblock %}
"""
...
<div class="flex items-start gap-6">
  <aside class="w-72 shrink-0 bg-white rounded-2xl shadow p-4 self-stretch">
...
def bootstrap_post(request: Request, full_name: str = Form(...), username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    if count:
        return RedirectResponse("/login", status_code=302)
    user = User(full_name=full_name, username=username, password_hash=hash_password(password), role="manager")
    db.add(user)
    db.commit()
    return RedirectResponse("/login", status_code=302)

@app.get("/dashboard", response_class=HTMLResponse)
def dashboard(request: Request, assignee_id: Optional[int] = None, status: Optional[str] = None, from_: Optional[str] = None, to: Optional[str] = None, db: Session = Depends(get_db), user: Optional[User] = Depends(get_current_user)):
    login_required(user)
...
def team_new(full_name: str = Form(...), username: str = Form(...), password: str = Form(...), role: str = Form("member"), db: Session = Depends(get_db), user: Optional[User] = Depends(get_current_user)):
    ...
    u = User(full_name=full_name, username=username, role=role, password_hash=hash_password(password))
    db.add(u)
    db.commit()
...
async def import_csv(file: UploadFile = File(...), db: Session = Depends(get_db), user: Optional[User] = Depends(get_current_user)):
    ...
    users = {u.username: u for u in db.exec(select(User)).all()}
    for row in reader:
